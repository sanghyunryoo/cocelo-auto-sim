...
bool ESKF::UpdateObserve(ESKF::ObsFunc obs) {
  ...
  for (int iter = 0; iter < options_.num_iterations_; ++iter) {
    ...
    const SO3 proposed_rotation = R_ * SO3::Exp(dx_.template block<3, 1>(0, 0));
    const V3 proposed_position = p_ + dx_.template block<3, 1>(3, 0);
    const double rotation_correction =
        (R_pred.inverse() * proposed_rotation).log_vee().norm();
    const double translation_correction = (proposed_position - p_pred).norm();
    const bool invalid_correction = !dx_.allFinite() ||
        !std::isfinite(rotation_correction) || !std::isfinite(translation_correction);
    const bool translation_rejected = options_.max_scan_correction_translation_ > 0.0 &&
        translation_correction > options_.max_scan_correction_translation_;
    const bool rotation_rejected = options_.max_scan_correction_rotation_rad_ > 0.0 &&
        rotation_correction > options_.max_scan_correction_rotation_rad_;
    if (invalid_correction || translation_rejected || rotation_rejected) {
      LOG(ERROR) << RED << " ---> [ESKF]: rejected scan correction: translation="
                 << translation_correction << " m, rotation="
                 << rotation_correction * 180.0 / M_PI << " deg." << RESET;
      R_ = R_pred;
      p_ = p_pred;
      v_ = v_pred;
      bg_ = bg_pred;
