...
def center_of_mass(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    com_tensor = asset.root_physx_view.get_coms().clone()[:, asset_cfg.body_ids].to(device)
...
def masked_height_scan(
    ...
    random_noise_range: tuple[float, float] | None = None,
    dropout_prob: float = 0.0,
    sample_hold_steps: int = 1,
    use_sensor_fov_mask: bool = False,
    frame_dropout_prob: float = 0.0,
    valid_probability_scale: float = 1.0,
    height_bias_by_index: tuple[tuple[int, float], ...] = (),
    valid_probabilities_by_index: tuple[tuple[int, float], ...] = (),
) -> torch.Tensor:
    """Height scan with lightweight optional noise/dropout."""
    sensor: RayCaster = env.scene.sensors[sensor_cfg.name]
    scan = sensor.data.pos_w[:, 2].unsqueeze(1) - sensor.data.ray_hits_w[..., 2] - offset
    if height_bias_by_index:
        scan = scan.clone()
        for index, bias in height_bias_by_index:
            if 0 <= int(index) < scan.shape[1]:
                scan[:, int(index)] += float(bias)
    if noise_std > 0.0:
        noise_scale = float(noise_std) + torch.abs(scan) * float(noise_std_per_meter)
        scan = scan + torch.randn_like(scan) * noise_scale
    if random_noise_range is not None:
        low, high = random_noise_range
        scan = scan + torch.empty_like(scan).uniform_(float(low), float(high))
    if dropout_prob > 0.0:
        keep = torch.rand_like(scan) >= float(dropout_prob)
        scan = torch.where(keep, scan, torch.zeros_like(scan))
    if frame_dropout_prob > 0.0:
        keep_frame = (torch.rand((scan.shape[0], 1), device=scan.device) >= float(frame_dropout_prob)).expand_as(scan)
        scan = torch.where(keep_frame, scan, torch.zeros_like(scan))
    return scan
...
def joint_pos_leg_gear_rel(
    ...
    """Return joint positions for the configured (leg) joints, scaled by `gear_ratio`."""
    asset: Articulation = env.scene[asset_cfg.name]
    delta_pos = asset.data.joint_pos[:, asset_cfg.joint_ids] - asset.data.default_joint_pos[:, asset_cfg.joint_ids]
    return delta_pos * gear_ratio
...
def joint_vel_leg_gear_rel(
    ...
    """Return joint velocities for the configured (leg) joints, scaled by `gear_ratio`."""
    asset: Articulation = env.scene[asset_cfg.name]
    delta_vel = asset.data.joint_vel[:, asset_cfg.joint_ids] - asset.data.default_joint_vel[:, asset_cfg.joint_ids]
    return delta_vel * gear_ratio
