...
def specific_joint_lower_limit_termination(
    ...
    """Terminate when any selected joint position is below a manual lower limit."""
    asset = env.scene[asset_cfg.name]
    joint_ids = asset.find_joints(list(joint_names))[0] if joint_names else asset_cfg.joint_ids
    return torch.any(asset.data.joint_pos[:, joint_ids] < float(threshold), dim=1)
