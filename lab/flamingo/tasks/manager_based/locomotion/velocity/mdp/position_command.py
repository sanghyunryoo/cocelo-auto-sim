...
class IntegralPositionCommand(CommandTerm):
    """Lightweight position command obtained by integrating the base velocity command."""

    def __init__(self, cfg: IntegralPositionCommandCfg, env: ManagerBasedEnv):
        super().__init__(cfg, env)
        self.robot: Articulation = env.scene[cfg.asset_name]
        self.position_command = torch.zeros(self.num_envs, 2, device=self.device)
        self.velocity_command = torch.zeros_like(self.position_command)

    @property
    def command(self) -> torch.Tensor:
        return self.position_command

    def _update_metrics(self):
        pos_error = self.position_command - self.robot.data.root_pos_w[:, :2]
        self.metrics["position_error"] = torch.norm(pos_error, dim=-1)

    def _resample_command(self, env_ids: Sequence[int]):
        self.position_command[env_ids] = self.robot.data.root_pos_w[env_ids, :2]
        self.velocity_command[env_ids] = 0.0

    def _update_command(self):
        base_command = self._env.command_manager.get_command(self.cfg.velocity_command_name)
        target_velocity = base_command[:, :2]
        max_delta = float(self.cfg.max_acceleration) * self._env.step_dt
        self.velocity_command += torch.clamp(target_velocity - self.velocity_command, -max_delta, max_delta)
        self.position_command += self.velocity_command * self._env.step_dt
...
class IntegralPositionCommandCfg(CommandTermCfg):
    """Configuration for the integrated position command."""
