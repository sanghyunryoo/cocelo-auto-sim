...
def error_track_pos_integral(
    ...
    """Position tracking error for the integrated position command."""
    asset: RigidObject = env.scene[asset_cfg.name]
    command = env.command_manager.get_command(command_name)[:, :2]
    error = torch.norm(command - asset.data.root_pos_w[:, :2], dim=1)
    error = torch.clamp(error - float(delta), min=0.0) * float(scale)
    if kernel == "exp":
        return 1.0 - torch.exp(-error)
    return error


def joint_deviation_zero_shoulder_l1(
    env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """Backward-compatible alias for shoulder joint deviation penalty."""
    return joint_deviation_zero_l1(env, asset_cfg)

def track_lin_vel_x_link_exp(
    env: ManagerBasedRLEnv, std: float, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """Reward tracking of linear velocity commands (x axis) using exponential kernel."""
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject = env.scene[asset_cfg.name]
    lin_vel_command = env.command_manager.get_command(command_name)[:, :1]

    # zero_mask = lin_vel_command.abs() <= 0.5
    # lin_vel_command = lin_vel_command * (~zero_mask).float()
    # compute the error
    lin_vel_error = torch.sum(
        torch.square(lin_vel_command - asset.data.root_link_lin_vel_b[:, :1]),
        dim=1,
    )
...
def still_lin_vel_xy_link_exp(
    ...
    is_zero_command = (command == 0.0)  # Check per item in batch if command is zero


    wheel_vel = asset.data.joint_vel[:, asset_cfg.joint_ids]
    wheel_vel_error = torch.sum(torch.abs(wheel_vel), dim=1)
    penalty = torch.exp(-wheel_vel_error / std**2)
    penalty = penalty * is_zero_command.float()
    return penalty

def still_lin_vel_x_link_exp(
    env: ManagerBasedRLEnv, std: float, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """Reward tracking of linear velocity commands (xy axes) using exponential kernel."""
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject = env.scene[asset_cfg.name]
    is_linx_zero_command = (env.command_manager.get_command(command_name)[:, 0] == 0.0)  # Check per item in batch if command is zero
    current_pos_z = asset.data.root_link_pos_w[:, 2]
    is_upright = (current_pos_z > 0.285)
    wheel_vel = asset.data.joint_vel[:, asset_cfg.joint_ids]    
    wheel_vel_error = torch.sum(torch.abs(wheel_vel), dim=1)
    linx_vel_penalty = torch.exp(-wheel_vel_error / std**2)
    penalty = linx_vel_penalty * is_linx_zero_command.float()
    penalty = penalty * is_upright.float()
    return penalty

def still_ang_vel_z_link_exp(
    env: ManagerBasedRLEnv, std: float, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """Reward tracking of linear velocity commands (xy axes) using exponential kernel."""
    # extract the used quantities (to enable type-hinting)
    asset: RigidObject = env.scene[asset_cfg.name]
    is_ang_z_zero_command = (env.command_manager.get_command(command_name)[:, 2] == 0.0)
    curent_pos_z = asset.data.root_link_pos_w[:, 2]
    is_upright = (curent_pos_z > 0.285)
    wheel_vel = asset.data.joint_vel[:, asset_cfg.joint_ids]   
    wheel_vel_diff_error = torch.abs(wheel_vel[:, 1] - wheel_vel[:, 0])
    angz_vel_penalty = torch.exp(-wheel_vel_diff_error / std**2)
    penalty = angz_vel_penalty * is_ang_z_zero_command.float()
    penalty = penalty * is_upright.float()
    return penalty
...
def track_zero_command_exp(
    ...
    # Get the current z position of the robot's base
    # Get the command z position relative to wheels from the command manager
    command_z = env.command_manager.get_command("base_velocity")[:, 3]
    is_zero_command = (command_z == 1.0)  # Check per item in batch if command is zero
    wheel_vel = asset.data.joint_vel[:, asset_cfg.joint_ids]
    wheel_vel_error = torch.sum(torch.abs(wheel_vel), dim=1)
    # Compute the error between the current height difference and the commanded height difference
    pos_z_error = torch.square(wheel_vel_error) * is_zero_command.float()
...
def reward_same_shoulder_z_position(
    ...
    for i in range(len(asset_cfg.body_ids)):
        shoulder_base[:,i,:] = quat_rotate_inverse(base_quat, shoulder_base[:,i,:])
...
def stand_still_base(
    ...
    """Penalize base motion/position drift only when:
    (1) command is zero AND (2) base height is above min_height.
    Height can be measured relative to terrain via RayCaster if provided.
    """
    asset: RigidObject = env.scene[asset_cfg.name]

    # 1) command == 0 마스크
    command_xy = env.command_manager.get_command(command_name)[:, :2]
    is_zero_command = torch.all(command_xy == 0.0, dim=1)

    # 2) 높이 마스크 (min_height 이상일 때만)
    # base height (prefer root_link_pos_w if available)
    if hasattr(asset.data, "root_link_pos_w"):
    ...
    if sensor_cfg is not None:
        sensor: RayCaster = env.scene[sensor_cfg.name]
        ground_z = torch.mean(sensor.data.ray_hits_w[..., 2], dim=1)
        height_rel = base_z - ground_z
    else:
        # flat/기본: env origin 대비 상대 높이로 판단
        height_rel = base_z - env.scene.env_origins[:, 2]

    is_high_enough = height_rel >= min_height

    # 최종 적용 마스크
    apply_mask = (is_zero_command & is_high_enough).float()
    ...
    position_error = torch.sum(torch.square(current_pos_xy - origin_xy), dim=1)

    position_penalty = position_error / (std**2)

    penalty = (velocity_penalty + position_penalty) * apply_mask
    return penalty
